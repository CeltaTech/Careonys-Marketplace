-- ---------------------------------------------------------------------------
-- 0016 — Un aviso guarda sus franjas como las guarda un Asistente
-- ---------------------------------------------------------------------------
-- Pendiente 40. `avisos.grid_schedule_7x3` es una columna `jsonb` y cada
...
--
-- **Esta migración era la 0015 y se renumeró el 25 de agosto de 2026**, porque
-- la 0015 pasó a ser el renombre del pendiente 52 y esta tabla tiene que
-- nacer después: cuelga de `avisos`, que antes se llamaba
-- `care_searches`. Se renumeró y no se agregó un renombre encima porque
-- todavía no se pegó en el panel de Supabase: no hay nada guardado que
-- corregir. `franjas_busqueda` nunca llegó a existir.
--
-- **Lo que esta migración no hace.** No vacía ni saca `grid_schedule_7x3`. Eso
-- toca datos ya guardados y lo decide el Desarrollador: hay avisos sembrados
-- por la migración 0003 y los que se hayan cargado probando. Desde hoy ninguna
...
create table if not exists public.franjas_aviso (
    ...
    unique (aviso_id, dia, turno)
);

comment on table public.franjas_aviso is
...
comment on column public.franjas_aviso.dia is
  'Clave del vocabulario dia_semana: lunes, martes, miercoles… Nunca «Lunes».';

comment on column public.franjas_aviso.turno is
...
-- --- 2. RLS con el mismo criterio que el aviso -------------------------------
-- `avisos` dice desde la migración 0002 que la fila es de la Prestadora
...
grant select, insert, update, delete on public.franjas_aviso to authenticated;
...
-- no lo publicó nadie.
revoke all on table public.franjas_aviso from anon;

-- --- 3. Los dos índices que la tabla va a necesitar --------------------------
-- El primero, para traer las franjas de un aviso. El segundo, para la
-- pregunta que motivó todo esto: quién hace falta los martes a la tarde.
create index if not exists idx_franjas_aviso_aviso on public.franjas_aviso(aviso_id);
create index if not exists idx_franjas_aviso_dia_turno on public.franjas_aviso(dia, turno);
