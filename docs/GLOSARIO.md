...
| **Anuncio** — identificador: `aviso` | Lo que una Familia publica cuando necesita un Asistente: qué necesita, en qué zona, qué días y en qué turnos. Queda publicado para que los Asistentes lo vean y lo respondan. **Es una cosa guardada, no algo que alguien hace**: el anuncio sigue existiendo aunque nadie lo mire. Ejemplo: una Familia publica que necesita acompañamiento los martes y jueves a la mañana en Caballito. No es la Búsqueda: la Búsqueda es la acción, el anuncio es lo que queda. Nunca se dice aviso: un aviso es lo que una persona avisa y queda registrado como acto suyo. Y lo que el producto le comunica a alguien —que llegó el Asistente, que alguien se postuló— es un **mensaje del sistema** | **Aprobado** por el Desarrollador el 25 de agosto de 2026, cuando ordenó «renombra todo lo que haya que renombrar, no puede ser que tengamos distintos nombres para la misma cosa» |
| **Búsqueda** — identificador: `busqueda` | La acción de buscar, y la hacen los dos lados de tres maneras: una Familia busca entre los Asistentes disponibles; una Familia también busca cuando publica un anuncio y espera que alguien se ofrezca; y un Asistente busca cuando revisa los anuncios publicados hasta encontrar uno que le cuadre. **Es lo que alguien hace, no algo que quede guardado**, y por eso no nombra ninguna tabla. Ejemplo: la Familia filtra por zona y patología, mira tres perfiles y no elige ninguno — hubo Búsqueda y no quedó nada | **Aprobado** por el Desarrollador el 25 de agosto de 2026, cuando ordenó «renombra todo lo que haya que renombrar, no puede ser que tengamos distintos nombres para la misma cosa» |
...
uno se postula a una Prestadora igual que se postula a un anuncio. Lo que cambia no es el verbo sino
...
`index.html` y en `solicitar-asistente.html` los Asistentes se postulan a un anuncio. Y en
...
justamente la postulación a un anuncio.
