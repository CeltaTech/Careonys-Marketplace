...
`supabase/migrations/0001_base_del_esquema.sql:466-488`, y ahí se ven una por una: el nombre en
`supabase/migrations/0001_base_del_esquema.sql:468`, el documento en `:469`, el teléfono en `:470`, el
...
dos columnas están declaradas en `supabase/migrations/0001_base_del_esquema.sql:2281-2282`.
...
—`supabase/migrations/0001_base_del_esquema.sql:4601-4603`—. Cuando no lo dice, no la ve
...
—`pwa-asistente/src/pantallas/Inicio/Fichador.jsx:199` y `:215`—. La respuesta que viene puesta es **no decirlo**
—`pwa-asistente/src/pantallas/Inicio/Fichador.jsx:205`—, y con ésa la marca no sale del propio Asistente. Terminada la
marca, el aviso de confirmación muestra las coordenadas que quedaron guardadas
—`pwa-asistente/src/pantallas/Inicio/Fichador.jsx:125-131`—. Aparte de eso está el pedido de permiso que hace el
...
`supabase/migrations/0001_base_del_esquema.sql:3084-3087`, y la tabla se llama por lo que es:
`reportes`.
...
—`supabase/migrations/0001_base_del_esquema.sql:160`—. Las del **Asistente** no son un dato de salud
de nadie: son la lista de lo que sabe atender, o sea su experiencia
—`supabase/migrations/0001_base_del_esquema.sql:474`—, y por eso, y sólo por eso, esas sí se
...
migración que lo crea: `supabase/migrations/0001_base_del_esquema.sql:6034`. Un papel de ésos no
...
—`supabase/migrations/0001_base_del_esquema.sql:6032`—, y el personal de la Prestadora de ese
legajo, y de ninguna otra
—`supabase/migrations/0001_base_del_esquema.sql:6036`—.

**6.4. La foto del Perfil es distinta, y hay que decirlo con todas las letras.** Está en un
depósito **público** —`supabase/migrations/0001_base_del_esquema.sql:6032`—, porque el directorio
...
antes de que exista una sola tabla —`supabase/migrations/0001_base_del_esquema.sql:63` y `:64`—.
Lo que sí contesta sin sesión son seis funciones que **exigen el nombre corto de una
Prestadora** —`supabase/migrations/0001_base_del_esquema.sql:5423`, `:5514`, `:5650`, `:5669`, `:5706` y `:5716`—, así que no
...
dicen qué parece un teléfono adentro del chat —`supabase/migrations/0001_base_del_esquema.sql:5852`, `:5861` y `:5885`—.
...
`supabase/migrations/0001_base_del_esquema.sql:749-760`.
...
—`supabase/migrations/0001_base_del_esquema.sql:758`—.
...
`supabase/migrations/0001_base_del_esquema.sql:423`, cuyo cuerpo entero es el renglón
`supabase/migrations/0001_base_del_esquema.sql:426-428`. **Nunca sale de algo que venga en el
pedido**, y así lo declara la propia base en
`supabase/migrations/0001_base_del_esquema.sql:433-435`. Al guardar tampoco se puede elegir:
la Prestadora se pone sola, porque es el valor por omisión de la columna
—`supabase/migrations/0001_base_del_esquema.sql:2739`—, así que ninguna pantalla puede elegirla, ni por error ni a
propósito.
...
—`supabase/migrations/0001_base_del_esquema.sql:4576-4578`—. **El motivo
...
—`supabase/migrations/0001_base_del_esquema.sql:1181`— y **no se autoasigna**: quien se registra
por su cuenta no puede tomar ese lugar, y así está escrito adentro de la función que crea el
perfil —`supabase/migrations/0001_base_del_esquema.sql:345-350`—. Desde una sesión, además, ni el rol ni la
...
—`supabase/migrations/0001_base_del_esquema.sql:580-581`—. Y guarda la
...
—`supabase/migrations/0001_base_del_esquema.sql:3554`—, así que queda el
...
—`supabase/migrations/0001_base_del_esquema.sql:4958-4960`—.
