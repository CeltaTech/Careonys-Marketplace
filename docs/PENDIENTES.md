...
| 27 | **La siembra ficticia no habla el idioma del catálogo.** `supabase/migrations/0003_dos_prestadoras_ficticias.sql` carga `profession` como `'enfermero'`, y la clave del vocabulario `perfil_profesional` es `enfermero_universitario`. Un filtro por perfil no devuelve esas filas, y el que busque la falla la va a buscar en el filtro | La siembra use las claves del catálogo. Es una migración de datos ficticios, así que se rehace entera sin costo |
| 28 | **`_mapToDatabase` pisa las patologías con el contacto.** En `js/apiClient.js`, la traducción de `care_searches` escribe `row.pathologies_required` dos veces: primero con las patologías y después, si vino `contacto`, con un arreglo que mezcla las patologías y un objeto `{ contacto }`. El teléfono de una familia termina guardado en la columna de las patologías | El contacto tenga su propia columna, o no se guarde. Hoy ninguna pantalla manda `contacto`, así que no hay datos mal guardados todavía |
