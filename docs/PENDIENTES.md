...
| # | Pendiente | Se cierra cuando |
|---|---|---|
| 18 | **Las banderas del legajo están declaradas, pero ninguna pantalla las pregunta y ninguna tabla las guarda.** Son cuatro —disponible para reemplazos urgentes, visible para las Familias, visible en los buscadores, comparte su teléfono— y las tres últimas son el consentimiento de la persona para que su perfil se muestre. **El texto, el orden y el momento ya están resueltos** en `data/catalogo-banderas.json`: se preguntan al terminar de cargar los datos, como último paso del alta, con una casilla cada una, y arrancan sin marcar. Falta lo que las hace reales | La pantalla de alta lea ese archivo y muestre el paso de cierre, el legajo guarde las cuatro banderas, y la vidriera no muestre a nadie que no tenga `perfil_publicado` en sí. **Es condición para cargar el primer legajo real**, igual que el pendiente 1 |
| 19 | **Las cuatro fichas repetibles están declaradas, pero ninguna pantalla las dibuja y ninguna tabla las guarda.** Matrícula, estudio o curso, experiencia laboral y referencia, con sus campos, sus validaciones y su texto en los tres idiomas, en `data/catalogo-fichas.json`. La Matrícula guarda organismo, número, vencimiento y archivo, que es lo que pedía el glosario y no existía | Las cuatro fichas se dibujen desde ese archivo y se guarden. **Un tipo de Asistente que exige Matrícula y no la puede cargar no se puede habilitar**, así que esto bloquea el alta de esos tipos |
| 20 | **No está decidido cuáles verificaciones bloquean el alta.** Son siete —documento, domicilio, título, matrícula, antecedentes penales, salud, referencia— y ninguna tiene dicho si es obligatoria, condicional u opcional. Es decisión de negocio, no de catálogo | El Desarrollador lo decida y quede escrito. Mientras tanto no se puede construir la pantalla de alta: el orden de los pasos depende de esto |
