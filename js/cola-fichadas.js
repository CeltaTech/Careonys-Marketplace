...
(function () {
  ...
  // sigue mostrando: una fichada trabada se ve, no desaparece.
  ...
  // Se le pasa la cuenta de las que están esperando a quien quiera mostrarla. La
  // pantalla se suscribe una vez y no pregunta.
  //
  ...
  function avisarLaCuenta() {
    ColaFichadas.cuantas().then((cuantas) => {
      avisados.forEach((quien) => {
        // Una pantalla rota no puede trabar la cola.
        try { quien(cuantas); } catch (e) { /* sigue */ }
      });
    }).catch(() => {});
  }
  ...
  const ColaFichadas = {
    ...
    async guardar(fichada) {
      const item = Object.assign({ intentos: 0, ultimoError: null }, fichada);
    ...
    //
    // Devuelve la lista de lo que salió mal, vacía si no salió mal nada. No
    // alcanza con que la cola se lo guarde para sí: un fallo que nadie puede
    // mirar es un fallo que no existe hasta que alguien pierde una fichada por
    // él. Quien la llama decide qué hacer con esa lista; hoy nadie la mira, y
    // eso es una decisión de pantalla, no de este archivo.
    async sincronizar() {
      const problemas = [];
      ...
      try {
        ...
        for (const fichada of cola) {
          ...
          try {
            ...
            fichada.intentos += 1;
            fichada.ultimoError = error && error.message ? error.message : String(error);
            await transaccion('readwrite', (deposito) => deposito.put(fichada));
            problemas.push(fichada.ultimoError);
            avisarLaCuenta();
            break;
          }
        }
        ...
        problemas.push(error && error.message ? error.message : String(error));
      } finally {
        sincronizando = false;
      }
      return problemas;
