...
const Sesion = {
  ...
  /* Un identificador distinto en cada llamada, y el único que los arma en
     todo el producto. Lo piden tres: el alta del Asistente y las fichas del
     legajo, que lo meten adentro del camino del papel que suben, y la cola de
     fichadas del teléfono, que se lo pone a cada marca antes de guardarla.

     Los dos del legajo vienen del pendiente 89, ya cerrado: armaban un camino
     que salía siempre igual —la cuenta y el tipo de papel, o la cuenta y la
     posición en la lista—, así que guardar de nuevo **borraba el archivo
     anterior**, sin preguntar y sin dejar rastro. Y el papel que se pisaba
     podía ser el que la Prestadora miró para otorgar el aval.

     La cola lo pide por otro motivo: con el identificador puesto de antemano,
     la fichada que sí llegó pero cuya respuesta se perdió en el camino no se
     duplica cuando se reintenta —la base la reconoce y la ignora—.

     `crypto.randomUUID` sólo existe en contexto seguro —`https` o
     `localhost`—, y servido sin cifrar es justo como se ve el sitio mientras
     se lo desarrolla. Abajo queda el mismo identificador armado a mano, con
     los dieciséis números al azar de donde salgan: del navegador si los da,
     y si no del azar común. La forma es la misma en los tres caminos, y eso
     no es un detalle: uno de estos identificadores termina siendo la clave de
     una fila, y una clave con otra forma la base la rechaza. */
  uuidNuevo() {
