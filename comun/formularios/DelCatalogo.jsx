...
export function SelectDelCatalogo({
  ...
  /* Lo elegido se repone cuando cambian las opciones, que es lo que hacía el
     `profesionSelect.value = elegido` de la página: el desplegable de Tipo de
     Asistente se completa dos veces —el vocabulario primero y el catálogo de
     fichas después—, y cambiarle las opciones a un desplegable le borra lo
     elegido. Lo que la persona ya había marcado no se pierde por eso. */
  useEffect(() => {
  ...
  return (
    <select ref={campo} id={id} className={className} required={required}
      ...
      {/* Las que ya están no se vuelven a poner: el mismo desplegable de Tipo de
          Asistente lo completa además el catálogo de fichas, que trae los tipos
          que declaran ficha propia. */}
      {extras
        .filter((e) => !items.some((i) => i.clave === e.clave))
        .map((e) => <option key={e.clave} value={e.clave}>{e.texto}</option>)}
    </select>
  );
}
